...
void TileView::SetTileLayerView(const TileLayer& depth, TileViewChild* child)
{
	GameObject::SetChildObj((GameObject*)child);
	m_LayerViews[(int)depth] = child;
	child->m_Layer = depth;
}
...
//		return;
//
//	auto tile = m_LayerViews[(int)TileLayer::Terrain]->m_TileDrawable[tileIndex.y][tileIndex.x];
...
void TileView::UpdateTileSprite()
{
	...
	{
		/*TileLayer& currdepth = m_SpriteUpdateQueue.front().first;
		sf::Vector2i& currIndex = m_SpriteUpdateQueue.front().second;
		auto& currTile = m_LayerViews[(int)currdepth]->m_TileDrawable[currIndex.y][currIndex.x];
