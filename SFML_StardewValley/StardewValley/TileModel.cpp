...
const TileInfo& TileModel::GetTileInfo(const TileLayer& depth, const CellIndex& tileIndex) const
{
	return m_TileInfos[(int)depth][tileIndex.y][tileIndex.x];
}

void TileModel::SetTiles(std::list<CellIndex>& tiles, const TileLayer& layer, const ID& id)
{
}
...
void TileModel::InitializeTileInfoLayer(const TileLayer& layer)
{
	...
	for (int j = 0; j < m_CellCount.y; j++)
	{
		for (int i = 0; i < m_CellCount.x; i++)
		{
			...
			currTileInfo.id = "OutdoorsSpring.png#Spring_X0Y7";
...
void TileModel::SetTile(const CellIndex& tileIndex, const TileLayer& depth, const ID& id, bool isTrueTile)
{
...
bool TileModel::IsPossibleToPass(const CellIndex& tileIndex)
{
	...
	{
		passable &= m_TileInfos[i][tileIndex.y][tileIndex.x].passable;
	}
	return passable;
}
