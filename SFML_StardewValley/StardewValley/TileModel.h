...
public:
	...
	void SetTiles(std::list<CellIndex>& tiles, const TileLayer& layer, const ID& id);
	//void SetTempEffectTiles(const CellIndex& tileIndex, TileType type, const std::string& subtype, const std::string& name);
protected:
	void SetTile(const CellIndex& tileIndex, const TileLayer& depth, const ID& id, bool isTrueTile = true);
public:
...
protected:
	...
	void RequestUpdateTile(const TileLayer& depth, const CellIndex& tileIndex);
